...
def sample_data(dhandlers,n_sample,device):
    '''
    sample `n_sample` data for each of the first `n_i` tasks from the `dhandlers` 
    '''
    sampled_data = []
    for dhandler in dhandlers:
        data = dhandler.next_train_batch(n_sample)
        X = dhandler.input_to_torch_tensor(data[0], device, mode='train')
        T = dhandler.output_to_torch_tensor(data[1], device, mode='train')
...
def get_embedding(metric, dhandlers, n_sample, device):
    ...
    sampled_data = sample_data(dhandlers=dhandlers, n_sample=n_sample, device=device)
